...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">
                    @if($whatsNew->platformVersion)
                        Edit feature for version {{ $whatsNew->platformVersion->version }} - {{ $whatsNew->platformVersion->title }}
                    @else
                        Edit feature
                    @endif
                </p>
            ...
            <div class="flex items-center space-x-3">
                @if($whatsNew->platformVersion)
                    <a href="{{ route('version-manager.versions.show', $whatsNew->platformVersion) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
    ...
    <div class="bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form method="POST" action="{{ route('version-manager.whats-new.update', $whatsNew) }}">
                @csrf
                @method('PUT')
                
                @if($whatsNew->platformVersion)
                    <input type="hidden" name="platform_version_id" value="{{ $whatsNew->platformVersion->id }}">
                @else
                    <div class="mb-6">
                        ...
                        <select name="platform_version_id" id="platform_version_id" required class="block w-full px-4 py-3 border-2 border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                            ...
                            @foreach($versions as $ver)
                                <option value="{{ $ver->id }}" {{ old('platform_version_id', $whatsNew->platform_version_id) == $ver->id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" 
                           name="title" 
                           id="title"
                           value="{{ old('title', $whatsNew->title) }}"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" 
                              ...
                              placeholder="Describe the feature...">{{ old('content', $whatsNew->content) }}</textarea>
                    @error('content')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="type" 
                            id="type" 
                            class="block w-full px-4 py-3 border-2 border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                        <option value="feature" {{ old('type', $whatsNew->type) === 'feature' ? 'selected' : '' }}>Feature</option>
                        <option value="improvement" {{ old('type', $whatsNew->type) === 'improvement' ? 'selected' : '' }}>Improvement</option>
                        <option value="bugfix" {{ old('type', $whatsNew->type) === 'bugfix' ? 'selected' : '' }}>Bug Fix</option>
                        <option value="security" {{ old('type', $whatsNew->type) === 'security' ? 'selected' : '' }}>Security</option>
                        <option value="deprecation" {{ old('type', $whatsNew->type) === 'deprecation' ? 'selected' : '' }}>Deprecation</option>
                    </select>
                    @error('type')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        @if($whatsNew->platformVersion)
                            <a href="{{ route('version-manager.versions.show', $whatsNew->platformVersion) }}" class="inline-flex items-center px-6 py-3 border border-gray-300 shadow-sm text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                Cancel
                            </a>
                        @else
