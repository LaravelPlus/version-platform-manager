...
@section('title', 'Edit What\'s New Entry')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Edit What's New Entry</h1>
...
createApp({
    data() {
        return {
            ...
            form: {
                platform_version_id: {{ $whatsNew->platform_version_id ?? '' }},
                title: '{{ $whatsNew->title ?? '' }}',
                content: '{{ $whatsNew->content ?? '' }}',
                type: '{{ $whatsNew->type ?? '' }}',
                sort_order: {{ $whatsNew->sort_order ?? '' }},
                is_active: {{ $whatsNew->is_active ? 'true' : 'false' }}
            },
            errors: {},
            submitting: false
        }
    ...
    methods: {
        ...
        async submitForm() {
            ...
            try {
                const response = await fetch('{{ route("version-manager.whats-new.update", $whatsNew->id) }}', {
                    method: 'POST',
                    headers: {
