...
final class WhatsNewController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($versionId) {
            $version = PlatformVersion::find($versionId);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('platform_version_id')) {
            $version = PlatformVersion::find($request->platform_version_id);
            if ($version) {
    ...
    public function importMarkdown(ImportMarkdownRequest $request, $platformVersionId)
    {
        ...
        foreach ($matches as $match) {
            ...
            $entryContent = mb_trim($entryContent);
            if (!$title || !$type || !$entryContent) {
                Log::warning('Skipping invalid entry', ['title' => $title, 'type' => $type, 'content_length' => mb_strlen($entryContent)]);

                continue;
            }

            // Validate and normalize status
            $normalizedStatus = mb_strtolower(mb_trim($status ?? 'draft'));
            if (!in_array($normalizedStatus, ['draft', 'private', 'published'])) {
                $normalizedStatus = 'draft';
            }

            // Validate and normalize type
            $normalizedType = mb_strtolower(mb_trim($type));
