...
final class AnalyticsService
{
    ...
    private function getVersionAdoption(): array
    {
        ...
        $versionCounts = $userVersions->groupBy('version')
            ...
            ->sortByDesc(fn ($count, $version) => version_compare($version, '0.0.0', '>') ? $version : '0.0.0');

        return $versionCounts->map(fn ($count, $version) => [
            'version' => $version,
            'users' => $count,
    ...
    private function getUpdateNotifications(): array
    {
        ...
        return $platformVersions->map(fn ($version, $index) => [
            'id' => $index + 1,
    ...
    private function getRecentActivity(): array
    {
        ...
        return $activities->sortByDesc(fn ($activity) => str_contains($activity['id'], 'v') ? 1 : 0)->take(5)->toArray();
    }
}
